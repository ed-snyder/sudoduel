...
function SudokuGrid({
  ...
  const prefilledCells = useMemo(() => {
    ...
    const cells: { row: number; col: number; value: number; index: number }[] = [];
    let index = 0;
    
    for (let row = 0; row < 9; row++) {
      for (let col = 0; col < 9; col++) {
        if (initialGrid[row]?.[col] !== 0) {
          cells.push({ row, col, value: initialGrid[row][col], index });
          index++;
        }
      }
    }
  ...
  return (
    <div
      ref={gridRef}
      className={`relative select-none gpu-accelerated ${animateIn ? 'grid-drawing-glow' : ''}`}
      style={{
        zIndex: 10,
        aspectRatio: '1 / 1',
        width: '100%',
      ...
      >
        {grid.map((row, rowIndex) =>
          row.map((cell, colIndex) => {
            ...
            // ONLY apply cell styling if NOT animating
            if (!isAnimatingIn) {
              // Only calculate "related" highlighting if NOT animating and a cell is selected
              const related = selectedCell && hasSelectedValue && (
