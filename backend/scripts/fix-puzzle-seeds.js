...
function getSeedsPath() {
    ...
    const scriptDir = path_1.default.resolve(process.cwd(), 'backend/scripts');
    return path_1.default.resolve(scriptDir, SEEDS_RELATIVE_PATH);
}
...
function validatePuzzle(initial, solution) {
    const errors = [];
    for (let i = 0; i < 81; i++) {
...
function validateSolutionGrid(grid) {
    const errors = [];
    // Rows
    for (let i = 0; i < 9; i++) {
        const seen = new Set();
...
function parsePuzzlesFromSql(sql) {
    const puzzles = [];
    const regex = /\(\s*1\s*,\s*'([0-9]{81})'\s*,\s*'([0-9]{81})'\s*,\s*'(EASY|MEDIUM|HARD|EXPERT)'\s*\)\s*[,;]/g;
    let match;
...
function rebuildSql(originalSql, puzzles) {
    ...
    const insertHeaderLine = 'INSERT INTO puzzles (ladder_id, initial_grid, solution_grid, difficulty) VALUES\n';
...
async function main() {
    ...
    const sql = fs_1.default.readFileSync(seedsPath, 'utf8');
    ...
    if (anyFixed) {
        ...
        fs_1.default.writeFileSync(seedsPath, newSql, 'utf8');
    ...
    const finalSql = anyFixed ? fs_1.default.readFileSync(seedsPath, 'utf8') : sql;
